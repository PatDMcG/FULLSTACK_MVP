...
function displayer(data) {
  for (i = 0; i < data.length; i++) {
    ...
    delet.addEventListener("click", async (e) => {
      let res = await fetch(`${API}chores/${e.target.id}`, {
        method: "DELETE",
      });
...
function edit(id) {
  ...
  while (document.getElementById("chores").lastChild) {
    document
      .getElementById("chores")
      .removeChild(document.getElementById("chores").firstChild);
  }
...
function reset() {
  ...
  document
    .getElementsByClassName("container_verticle")[0]
    .removeChild(document.getElementById("KILL"));
  document
    .getElementsByClassName("container_verticle")[0]
    .removeChild(document.getElementById("UI"));
  console.log(
    document
      .getElementsByClassName("container_verticle")[0]
      .removeChild(
        document.getElementsByClassName("container_verticle")[0].lastChild
      )
  );
  while (document.getElementById("chores").lastChild) {
    document
